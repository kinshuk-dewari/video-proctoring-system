...
export async function GET(
  req: NextRequest, // first argument must be the request object
  context: { params: { reportId: string | string[] } } // ← allow string[]
) {
  const reportId = Array.isArray(context.params.reportId)
    ? context.params.reportId[0] // take first if multiple
