...
const FinalProctoring: React.FC<FinalProctoringProps> = ({ sessionId, candidateName }) => {
  ...
  const runDetection = async () => {
    ...
    if (faceModelRef.current && objectModelRef.current) {
      ...
      if (faces.length > 1) logEvent("FocusDetection", "Multiple faces detected");
      ...
      objects.forEach(obj => {
        if (["cell phone", "book", "laptop", "mouse", "remote", "keyboard"].includes(obj.class)) {
          logEvent("ObjectDetection", `${obj.class} detected`);
        }
      });
    }
  };
...
export default FinalProctoring;
...
// import { renderPredictions as renderObjects } from "@/lib/render-predictions";
// import { renderPredictions as renderFace } from "@/lib/render-predictions-blazing";
// import { isLookingAway } from "@/lib/proctoring-utils";
// import { createNamedLogger } from "@/lib/event-logger";
...
//       if (isCanvasVisible) renderFace(faces, ctx);
...
//       const objects = await objectModelRef.current.detect(video, undefined, 0.6);
//       if (isCanvasVisible) renderObjects(objects, ctx);
...
//           </div>
//           <Button1 onClick={() => setIsCanvasVisible(prev => !prev)}>
//             {isCanvasVisible ? "Hide Overlay" : "Show Overlay"}
//           </Button1>
//         </div>
//       )}
//     </div>
